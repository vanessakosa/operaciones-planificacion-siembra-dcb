...
| 🔨 | `roles_cartera.csv` | — | **NUEVO 2026-09-10.** El rol de cada grupo en la cartera: `BASE` (siempre debe haber) · `BASE_ENCAJE` (Ammi y Trachelium se sustituyen) · `FOCAL` (siempre una principal) · `TOQUE` (rota a propósito, poca cantidad, Inv 2) · `TOQUE_ENSAYO` (con criterio de salida). Dictado por Vanessa. Sin este archivo la cartera compara grupos que no compiten entre sí. 10 grupos clasificados, **14 sin rol** — no se asignan por deducción. Ver `13-optimizacion/03-estrategia-de-surtido.md` |
