#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Calibra el rendimiento teorico contra el que de verdad dio el campo.

    python3 motor/calibrar_rendimiento.py

Compara, grupo por grupo, dos numeros:

    OBSERVADO  tallos reales / plantas-semana que estuvieron en cosecha
    TEORICO    tallos_planta / ventana_sem_max, de ciclos_variedad.csv

El ratio entre los dos es el factor de correccion que hay que aplicarle a una
proyeccion hecha con el ciclo para que se parezca a la realidad. Un ratio de
0,5x quiere decir que el ciclo promete el doble de lo que la finca entrega.

El denominador son PLANTAS-SEMANA, no plantas: una cama de 1.000 plantas que
estuvo seis semanas en cosecha aporta 6.000 plantas-semana. Es la unidad que
permite comparar un lote que produjo dos semanas con uno que produjo diez.

Solo mira las semanas ISO 22 a 33 de 2026, que es lo que cubre el registro de
tallos. Las siembras de 2025 cuya cosecha no cruzo el ano quedan fuera.

VIENE DE LA RAMA harvest-gaps-analysis-1cofhy (2026-09-03), donde leia un
'puente.csv' que era una copia de trabajo de campo_siembras.csv y que nunca se
commiteo. Reapuntado a la fuente real: las posiciones de columna que usaba
(2 variedad, 5 trasplantada, 6 fecha siembra, 7 semana siembra, 10 inicio de
cosecha, 11 fin) son exactamente las de campo_siembras.csv.
"""
import csv, collections, datetime, json, os
RAIZ = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
P = os.path.join(RAIZ, '07-datos') + os.sep
C={c['grupo']:c for c in csv.DictReader(open(P+'ciclos_variedad.csv',newline='',encoding='utf-8'))}
...
rows=list(csv.reader(open(P+'campo_siembras.csv',newline='',encoding='utf-8')))[1:]
lot=[]
for r in rows:
    var=r[2].strip()
    if not var: continue
...
for x in csv.DictReader(open(P+'registro_tallos.csv',newline='',encoding='utf-8')):
    f=x['Fecha']
...
SALIDA = os.path.join(RAIZ, '07-datos', 'factores_rendimiento.json')
json.dump(res, open(SALIDA, 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
print('factores escritos en', os.path.relpath(SALIDA, RAIZ))
print()
